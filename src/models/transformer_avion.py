    ...
    def forward(self, x: torch.Tensor):
        if self.use_fast_conv1:
            if self.num_frames == 1:
                ...
                # old code
                # here it expects (1, 3136, 768), 3136 is the num of patches and 768 = 3 * 16 * 16 is each patch
                # x = rearrange(x, "b c t (hh sh) (ww sw) -> b (t hh ww) (c sh sw)", sh=self.patch_size[0], sw=self.patch_size[1])

                # new code after adding tokenizer
                # input [1568, 3, 2, 16, 16]
                x, token_mask, _ = x['split_crops'], x['token_mask'][0], x['pos_embeds']
               
                x = rearrange(x, "(thw) c 1 p1 p2 -> 1 (thw) (c p1 p2)") # correct

                x = self.conv1(x)
                total_pos_embed = torch.zeros_like(x)[0]
                x = torch.cat(
                    [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
                    x], dim=1)  # shape = [*, grid ** 2 + 1, width]
                ...
                # not sure 
                # use token_mask to select from tile_pos_embed
                tile_temporal_embed = self.temporal_embedding.repeat_interleave(self.patches_per_frame, 0)
                for embed in [tile_pos_embed, tile_temporal_embed]:
                    total_pos_embed += embed[token_mask]
